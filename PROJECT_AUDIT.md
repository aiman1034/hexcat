...
| **MikroTik Switches** | 36 | **non-weight 0 violations**; weight guard correctly hard-fails 36/36 placeholder Artikelgewicht (2026-06-14, commit `33e3e8d`) | **0×8** + S.1-S.6 | 36/36 MSRP captured; EUR deferred | ⏸ **HELD on weight-grounding** — 1st SWITCH brand (Rule-7). schema/combo/access/S.1-S.6/B.1-B.8 all PASS; env-first L3 Managed(L3) 20/Smart 7/Industrie 3/DC 6; temp datasheet-verbatim. Real switch weights NOT $0-reachable (product page/specs/manual/Confluence REST API all verified to omit it; JS-rendered, browser binary not installed) → §10 weight dependency. Guard blocks the optics placeholder; **no clean ZIP re-emitted until weights land.** Completeness 36 of 40 (4 non-switches excluded). |
...
| **MikroTik switches — WEIGHTS** | mikrotik.com per-product spec widget (`Weight`) | content is **fully authored + every other attribute grounded**, but the per-product weight is **JS-rendered only** — verified absent from the product page, specs page, manual page, and Confluence REST API on $0 GET; Playwright Chromium binary not installed (browser fetch = operator-gated/CAPTCHA-opt-in). Per 1000%-grounding the placeholder is not shipped → the **weight guard hard-fails all 36**. | `mikrotik-switch-weights.{csv,yaml}` (PN → Artikelgewicht kg; Versandgewicht optional) — OR approve the browser/CAPTCHA fetch (the rendered widget HAS the weight). Once dropped, the batch re-emits clean immediately (all other work done). |

**SWITCH sources (same gating):** MikroTik switches are **authored + verified except weights** (the one
residual $0 blocker — see the WEIGHTS row above; HELD until a weight source lands). Every OTHER brand's switches
