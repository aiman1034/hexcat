...
| **Juniper** | **172 modules enumerated** (not grounded) | **enumeration built — NOT grounded, NOT authorable** | core. Enumerated universe = union of 2 independent distributors (edgeoptic 87 + AddOn 116, genuine Juniper PNs only, house codes stripped, deduped, modules-only): **172 transceiver modules** (+14 cables/0 converters tracked separately). Tiers 1G→800G (10G 40, 100G 33, 25G 26, 40G 22, 1G 12, 400G 10…); families EX-SFP 44, JNP-SFP 36, JNP-QSFP 21, QDD 16, QSFP-100G 12… (`juniper_enumerated_universe.json`). 2-source union (HPC 403, FS/QSFPTEK pending → not locked). NEXT (after operator picks ship-path): +3rd source → GROUND each from Juniper datasheets → Workflow-B-to-empty → author. |
