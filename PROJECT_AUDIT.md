...
| **Juniper** | 38 (facts, legacy) | **facts** (legacy SFP/SFP+/XFP/XENPAK) — modern set blocked | core. `juniper_transceivers_facts.json` = 38 grounded Ethernet optics (SFP 22/SFP+ 4/XFP 8/XENPAK 4; 23 EOL-flagged-kept); 10 SONET excluded (scope decision). **Modern QSFP/100G/400G = harvest gap** (JS qualified-optics pages time out headless even on domcontentloaded) → alternate source next (HCT JSON API / current optics PDF / per-platform doc tables). Author Juniper as ONE batch after the modern set lands. |
