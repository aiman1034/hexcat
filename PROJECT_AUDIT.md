...
- **CHECKPOINT 2026-06-14 #2 (context budget, after B.8):** B.8 done + 6 brands re-verified at audit_semantic **0×8** + ZIPs re-emitted (HPE/Fortinet/Arista `…_f8fd859.zip`). Stopping before the NVIDIA AUTHORING phase (85 fresh SKUs of grounded German = the context-heavy work; starting it now would risk a half-built brand). **NVIDIA enumeration FINALIZED this session:** `_scratch/nvidia_harvest.py` → **85 Ethernet SKUs** (400/200/100/25G LinkX list, IB filtered out, edge-case asserts pass). The 800G list (`nvidia_parts_800g.txt`) is **deprecated + XDR InfiniBand, ZERO Ethernet parts → correctly OUT OF SCOPE** (we catalog Ethernet); nothing to fold in. So NVIDIA Ethernet universe = **85 of 85**. The other 10 fresh brands have no harvest yet. **Resume point:** start NVIDIA — build a per-SKU facts file from the parts-list rows (Form Factor / Configuration / Length / PN columns are structured + grounded), write `nvidia_author.py` (scaffold the gold-slice: artikelname/titel/meta/kurz 40–80w/beschreibung 90–175w/intro[3]/faq, varied per-SKU by specs) → `backfill_brand` for Anwendung/Betriebstemperatur(Rule-9 commercial — NVIDIA DC optics) → gate PASS → Rule-8 parity → audit_semantic 0×8 → price (no list → 0,00) → commit per family → PROJECT_AUDIT → ZIP. Then the remaining 10 fresh brands the same way; then switches (Rule-7 STOP).
