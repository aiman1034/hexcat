...
- **2026-06-29 — GROUNDING (research only, not built): Cisco Nexus 5000 first-gen fixed switches (5010 + 5020).** Enumerated the orderable base-chassis PIDs from the original Nexus 5000 datasheet (`data_sheet_c78-461802`, transcribed verbatim from two independent Cisco-PDF mirrors `data.nag.wiki/.../N5K_5000_datasheet.pdf` + telecomdatasheets/145.pdf; cisco.com 403, Wayback blocked). **Exactly 2 fixed PIDs:** `N5K-C5010P-BF` ("Cisco Nexus 5000 1RU Chassis no PS, 2 Fan Modules, 20 ports (req SFP+)") and `N5K-C5020P-BF` ("…2RU Chassis no PS, 5 Fan Modules, 40 ports (req SFP+)"). **NO -BR / reversed-airflow base chassis exists** in Table 8 — first-gen is single fixed front-to-back cooling with rear ports (selectable airflow is a 5500-gen feature). Grounded verbatim: **5010** = 20× 10G-SFP+ (Eth/FCoE; ports 1–8 also 1G), 1 expansion slot, SwK **520 Gbit/s / 386,9 Mpps**, 1RU, 1+1 redundant AC 550 W, 0–40 °C, **35 lb (15,88 kg)** w/ 2 PSU+1 expansion+2 fans. **5020** = 40× 10G-SFP+ (Eth/FCoE; ports 1–16 also 1G), 2 expansion slots, SwK **1,04 Tbit/s / 773,8 Mpps**, 2RU, 1+1 redundant AC 750/1200 W, 0–40 °C, **50 lb (22,68 kg)** w/ 2 PSU+2 expansion+5 fans. SwK is the Cisco "Layer 2 hardware forwarding" figure NOT doubled. **Nexus-lane confirmations:** VXLAN = **0 hits** (absent, pre-VXLAN-HW) → Layer **L2** (L3 only via add-on, not in the original DS feature list → prose, not Merkmal); **FCoE IS present** on first-gen (the original 10GbE+FCoE value prop) but ports are **NOT Unified** — native FC only via expansion modules, so FCoE→prose, no Merkmal; unified ports absent (those arrived with 5548UP). PoE Nein, Stacking Nein (vPC). Weight is the **full-config datasheet value** (incl. PSUs+expansion+fans) → flag bare-chassis lighter = ZU_VERIFIZIEREN, same convention as the 5500 sibling. **EoS/EoL:** EoL-announced, last order **27-Nov-2012**, last ship **25-Feb-2013**, End-of-RFA **27-Nov-2013**, Last-Date-of-Support **28-Nov-2017** (`eol_c51-709037`; flag-don't-drop). **5696Q (N5K-C5696Q) classified = MODULAR, NOT fixed** — 4RU / 8 line-card-expansion-module (LEM) slots (5696-M12Q 12×40G GEM, N5696-M20UP, 100G CXP) → chassis carve-out, excluded from this fixed lane (matches the M-1 note "Nexus …5696Q carve-outs"). Schema mirrors the built `output/switches/Cisco_Nexus_5500/`. **Not emitted; awaits build go.**
...
- **CISCO SWITCH LONG-TAIL — RECONCILE then BUILD (2026-06-29). 6 missing manifest rows → 10 fixed switches built + 2 corrected;
  983→993 built; only 2 rows left missing (both legitimate).** Content-only (src-diff EMPTY). PHASE 1 reconcile (measure-first,
  4 grounding agents) classified the 6 rows: (1) **Meraki MS (~30 TBD)** = DUPLICATE-MERGE (overlapped the built 68-SKU rack lane +
  row 5) → retired; (2) **Nexus 9800** = PARK (chassis-modeling decision pending — untouched); (3) **Nexus 3000 remaining** = mixed
  → BUILD N3K-C3132C-Z, VERIFY 34200YC-SM, carve-out 3408-S, exclude superseded -10GE/-10G; (4) **Catalyst 4948E-F** = CONFIRMED →
  BUILD; (5) **Meraki MS desktop** = BUILD-5; (6) **Nexus 5000 5500/5600** = mixed → BUILD 5010/5020, carve-out 5696Q. PHASE 2 BUILT
  (standard switch schema, all gate ok=True/0/0): **Cisco_MS130_Desktop** (5: MS130-8/-8P/-8P-I/-8X/-12X; Bauform=Desktop existing,
  Kühlung Passiv/Aktiv; -8P-I = INTERNAL PSU not industrial; S.3 holds with mGig; Line-Rate Durchsatz; grounded 0,88-1,3 kg);
  **Cisco_Nexus_5000** (2: N5K-C5010P-BF/5020P-BF first-gen; L2, FCoE→prose not unified, VXLAN absent; weight datasheet FULL-CONFIG,
  bare ZU_VERIFIZIEREN; EoS 2012); **Cisco_Nexus_3132C_Z** (1: N3K-C3132C-Z 100G BiDi; L3, VXLAN+MPLS/VPLS/GRE/Q-in-Q grounded→prose,
...
- **HPE/Aruba SWITCHES — STEP 2 BATCH-4b: CX 10040 authored (2026-06-30; batch-4 deferral resolved).** Auditor couriered the full
  ...
  2U-class, flagged), fan count (defaulted 4; doc BtF=6 likely typo), 2× SFP+ modeled as mgmt. **OPERATOR DECISION (FRU): INCLUDE** —
  S4R58A + S4R59A (TAA) authored as sealed-switch bare-chassis FRU (configurable airflow, PSU/fans separate, like the base/spare units);
...
- **MERAKI = CISCO BRAND (operator 2026-07-01; Hersteller flip).** Operator clarified Meraki is a Cisco brand (like Aruba is
  an HP brand), so the 23 Meraki transceivers' **Hersteller Meraki → Cisco**. rules.yaml `"Meraki"` vendor entry → `{hersteller:
  "Cisco", slug: "meraki"}` (slug kept `meraki` so URL-Pfad `meraki/<sku>` is unchanged; the slug↔Hersteller map now resolves
  meraki→Cisco so URL-Pfad validation still holds). Prose already said "Cisco Meraki" (69×). Applied via **pure byte replace**
