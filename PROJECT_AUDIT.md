...
- Next: 800G-Eth follow-up harvest (when a $0 parts list is locatable), then the remaining 10 fresh brands (Avaya/Extreme, Dell, Huawei, Juniper, Lenovo/IBM, Palo Alto, Ruijie, Supermicro, Ubiquiti, ZTE) — same facts→author→backfill→gate→ZIP pattern; then **switches (Rule-7 schema STOP).**
